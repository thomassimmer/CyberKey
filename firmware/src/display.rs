...
/// Show factory-reset success ("Reset OK" in green + "Rebooting..." in white).
pub fn show_reset_ok<D: DrawTarget<Color = Rgb565>>(display: &mut D) {
    let _ = display.clear(Rgb565::BLACK);
    let ts = TextStyleBuilder::new().alignment(Alignment::Center).build();
    let cx = (W / 2) as i32;
    let cy = (H / 2) as i32;
    let ok_style = MonoTextStyle::new(&FONT_10X20, Rgb565::GREEN);
    let _ = Text::with_text_style("Reset OK", Point::new(cx, cy - 12), ok_style, ts).draw(display);
    let sub_style = MonoTextStyle::new(&FONT_6X13, Rgb565::CSS_LIGHT_GRAY);
    let _ =
        Text::with_text_style("Rebooting...", Point::new(cx, cy + 12), sub_style, ts).draw(display);
}
